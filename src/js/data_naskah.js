  ...
  .then((data) => {
    ...
    deleteButtons.forEach((button) => {
      button.addEventListener("click", (event) => {
        const idnaskah = event.target.getAttribute("id-naskah");
    ...
    detailButtons.forEach((button) => {
      button.addEventListener("click", (event) => {
        const idnaskah = event.target.getAttribute("id-penulis");
        window.location.href = `edit_order.html?order_id=${idnaskah}`;
      });
    });
  })
  .catch((error) => {
